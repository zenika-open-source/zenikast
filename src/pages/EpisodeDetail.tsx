        ...
        <div className="flex flex-col h-full bg-[#1e1a1b] p-8 pb-10">
          ...
          <div className="flex flex-col sm:flex-row gap-8 mb-8">
            ...
            <div className="flex-1 flex flex-col justify-center">
              ...
              <div className="flex flex-wrap gap-2">
                <a href={episode.spotifyUrl || "#"} target={episode.spotifyUrl ? "_blank" : "_self"} rel="noreferrer" className="flex items-center gap-2 bg-[#1A1A1A] hover:bg-[#2A2A2A] transition-colors border border-white/5 rounded-full px-4 py-2 text-sm font-medium">
                ...
                <a href={episode.youtubeUrl || "#"} target={episode.youtubeUrl ? "_blank" : "_self"} rel="noreferrer" className="flex items-center gap-2 bg-[#1A1A1A] hover:bg-[#2A2A2A] transition-colors border border-white/5 rounded-full px-4 py-2 text-sm font-medium">
                ...
                <a href={episode.deezerUrl || "#"} target={episode.deezerUrl ? "_blank" : "_self"} rel="noreferrer" className="flex items-center gap-2 bg-[#1A1A1A] hover:bg-[#2A2A2A] transition-colors border border-white/5 rounded-full px-4 py-2 text-sm font-medium">
                  <div className="w-5 h-5 flex items-center justify-center text-[#8e43e7]">
                    <Mic className="w-4 h-4 fill-current" />
                  </div>
                  Deezer
                </a>
                <a href={episode.appleUrl || "#"} target={episode.appleUrl ? "_blank" : "_self"} rel="noreferrer" className="flex items-center gap-2 bg-[#1A1A1A] hover:bg-[#2A2A2A] transition-colors border border-white/5 rounded-full px-4 py-2 text-sm font-medium">
          ...
          <div className="text-lg text-white/70 leading-relaxed font-light mb-10">
            {episode.longDescription ? (
              <div dangerouslySetInnerHTML={{ __html: episode.longDescription.replace(/\n/g, '<br />') }} />
            ) : (
              episode.description
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-8 gap-x-12">
            <div className="sm:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-12">
              <div className="sm:col-span-2 text-brand-red text-xs font-bold tracking-widest uppercase mb-1 text-[#f25f5a]">INVITÉ•E•S</div>
              {episode.guestsList ? episode.guestsList.map((guest: any, i: number) => (
                <div key={i}>
                  <div className="font-bold text-white uppercase">{guest.name}</div>
                  <div className="text-white/50 text-sm">{guest.role}</div>
                </div>
              )) : (
                <div className="sm:col-span-2">
                  <div className="font-bold text-white uppercase">{episode.guests}</div>
                </div>
              )}
            </div>
            ...
            <div className="sm:col-span-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-12">
                {episode.hostsList ? episode.hostsList.map((host: any, i: number) => (
                  <div key={i}>
                    <div className="font-bold text-white uppercase">{host.name}</div>
                    <div className="text-white/50 text-sm">{host.role}</div>
                  </div>
                )) : (
                  <div className="sm:col-span-2">
                    <div className="font-bold text-white uppercase">{episode.commentedBy}</div>
                  </div>
                )}
              </div>
            </div>
